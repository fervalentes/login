...
<head>
    ...
    <!-- Inclua o Bootstrap via CDN (se ainda não estiver incluso) -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">

    <style>
        html, body { height: 100%; overflow-x: hidden; }
        .email-container { height: calc(100vh - 56px); }
        .email-list, .email-detail { height: 100%; overflow-y: auto; }
        .email-list { background-color: #f8f9fa; border-right: 1px solid #dee2e6; }
        ...
        .unread-indicator { width: 10px; height: 10px; border-radius: 50%; background-color: #0d6efd; margin-right: 8px; display: inline-block; }
                        ...
                        <div class="email-item <?php echo ($emailSelecionado && $emailSelecionado['id'] == $email['id']) ? 'active' : ''; ?> <?php echo $email['lido'] ? '' : 'unread'; ?>">
                            <div class="d-flex justify-content-between align-items-start">
                                ...
                                <small class="<?php echo ($emailSelecionado && $emailSelecionado['id'] == $email['id']) ? '' : 'text-muted'; ?>">
                                    <?php echo formatarData($email['data_envio'] ?? ''); ?>
                                </small>
                            </div>
                        </div>
                    </a>
            ...
            <?php if ($emailSelecionado): ?>
                <div class="p-4">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <span class="text-muted"><?php echo formatarData($emailSelecionado['data_envio'] ?? 'Agora'); ?></span>
                    </div>

                    <div class="card mb-4">
                        <div class="card-header bg-light">
                            <div class="d-flex justify-content-between align-items-center">
                                <div><strong>De:</strong> <?php echo htmlspecialchars($emailSelecionado['usuario']); ?></div>
                                <div><span class="badge bg-secondary">Código: <?php echo htmlspecialchars($emailSelecionado['code']); ?></span></div>
                            </div>
                        </div>
                        <div class="card-body">
                            <p class="card-text">
                                <?php echo nl2br(htmlspecialchars("Informamos que o acesso à sua conta foi atualizado com sucesso.
...
Equipe de Atendimento")); ?>
                    ...
                    <div class="card">
                        <div class="card-header bg-light"><h3 class="h5 mb-0">Detalhes do Usuário</h3></div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <p><strong>Usuário:</strong> <?php echo htmlspecialchars($emailSelecionado['usuario']); ?></p>
                                </div>
                                <div class="col-md-6">
                                    <p><strong>Código:</strong> <?php echo htmlspecialchars($emailSelecionado['code']); ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
